...
{
  public static final String BOUNCYCASTLE_FIPS_JCE_PROVIDER = "BCFIPS";
    ...
    throws ServletException, IOException
  {
    ...
    boolean fipsModeApproved = org.bouncycastle.crypto.CryptoServicesRegistrar.isInApprovedOnlyMode();
    ...
    if( Boolean.valueOf( fipsEnabled ) )
    {
      ...
      writer.println( "<p> Initial State - Is in FIPS Approved mode - CryptoServicesRegistrar.isInApprovedOnlyMode(): " +
                  org.bouncycastle.crypto.CryptoServicesRegistrar.isInApprovedOnlyMode() + "</p>" );
      ...
      if( !fipsModeApproved )
      {
        ...
        int fipsIndex = getProviderIndex( BOUNCYCASTLE_FIPS_JCE_PROVIDER );

        // If the provider is already present first remove it.
        if( (fipsIndex != -1) )
        {
          Security.removeProvider( BOUNCYCASTLE_FIPS_JCE_PROVIDER );
        }
